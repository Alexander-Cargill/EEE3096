...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  HAL_Delay (LedPeriod); // wait for 500 ms * 3.6/4096

	  uint32_t AdcVal = pollADC();
	  uint32_t CrrVal = ADCtoCRR(AdcVal);
	  sprintf(buffer, "%6d\r\n", CrrVal);
...
uint32_t ADCtoCRR(uint32_t adc_val){
	...
	float ratio = (adc_val * 10000)/4096;
	uint32_t val = (uint32_t) (47999 * ratio)/10000;
